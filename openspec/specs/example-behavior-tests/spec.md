...
The `blue-square` curated example SHALL have a headless behavioral test that scripts movement input, drives one or more frames through `generated_update_project`/`generated_render_project` without a window, and asserts against the fake raylib call log that the expected shape draw call occurred with the expected position, size, and color.
