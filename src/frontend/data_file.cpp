...
EntityInstanceData DataFileWriter::build_record(const std::string& name,
...
void DataFileWriter::add_child_records(std::vector<EntityInstanceData>& records,
    ...
    for (const auto& grandchild : child.children) {
        add_child_records(records, grandchild, node_name);
    }
}
    ...
    for (const auto& decl : ast_.declarations) {
        if (const auto* entity = std::get_if<EntityNode>(&decl)) {
            ...
            for (const auto& child : entity->children) {
                add_child_records(records, child, entity->name);
            }
        }
