...
Automated example-compilation integration coverage SHALL include the gradient square example as a named curated `cpp-entt` case.

#### Scenario: Curated example list includes gradient square

- **WHEN** `ExampleCppCompilationTests` enumerates curated examples
- **THEN** it includes a case named `gradient-square` with source `examples/gradient-square/gradient_square.cactus` and backend `cpp-entt`
