...
The repository SHALL provide `examples/mesh-renderer/main.cactus` as a valid Cactus DSL example that imports the stdlib input and 3D mesh-rendering surfaces and uses authored systems to rotate the demo mesh from WASD input at runtime with screen-space control semantics.
...
#### Scenario: Example updates mesh rotation from authored systems
- **WHEN** the example's runtime systems execute after WASD input changes
- **THEN** the demo mesh entity's volume transform rotation is updated by authored Cactus systems rather than project-local C++ callbacks
