...
CommandResult run_command(const fs::path& working_dir,
...
[[noreturn]] void fail_stage(const ExampleCase& example,
                             const std::string& stage,
...
std::string build_target_command(const std::string& target) {
    ...
    command << quote(fs::path(CACTUS_CMAKE_COMMAND))
            << " --build " << quote(build_root())
            << " --target " << quote(target);
    ...
    for (const auto& example : curated_examples()) {
        SECTION(example.name) {
            ...
            require_success(
                ...
                "generate",
                repo_root(),
                quote(compiler_path()) + " " + quote(example.source_file) +
                    " --backend " + quote(example.backend) +
            ...
            require_success(
                ...
                "compile",
                repo_root(),
                build_target_command(example.compile_target));
            ...
            require_success(
                ...
                repo_root(),
                quote(fs::path(CACTUS_CLANG_FORMAT_PATH)) +
                    " --dry-run --Werror " + quote(example.generated_cpp));

            require_success(
                example,
                "clang-tidy",
                repo_root(),
                quote(fs::path(CACTUS_CLANG_TIDY_PATH)) +
                    " " + quote(example.generated_cpp) +
                    " -p " + quote(build_root()) +
                    " --header-filter=" + quote(example.generated_cpp.string()) +
                    " --warnings-as-errors=*" +
