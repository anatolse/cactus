...
- [ ] 1.1 Add `EXTERN` to the `TokenType` enum in `src/frontend/token.hpp`
...
- [ ] 2.1 Add `bool is_extern = false` field to `FuncNode` in `src/frontend/ast.hpp`
...
- [ ] 4.1 Add `ResolvedParam` and `ResolvedFunc` structs to `src/frontend/semantic_analyzer.hpp`
- [ ] 4.2 Add `std::unordered_map<std::string, ResolvedFunc> funcs` to `DecoratedProgram` in `src/frontend/semantic_analyzer.hpp`
- [ ] 4.3 Add `std::unordered_map<std::string, ResolvedFunc> funcs` to `ImportedSymbols` in `src/frontend/semantic_analyzer.hpp`
...
- [ ] 5.1 Increment `CURRENT_VERSION` from `1` to `2` in `src/frontend/module_artifact.hpp`
...
- [ ] 6.2 If `has_extern_funcs` returns true, emit `#include "cactus_runtime.hpp"` after the fixed system includes
...
- [ ] 6.4 Add codegen test: program with extern func generates file containing `#include "cactus_runtime.hpp"`; program without extern func does not
...
- [ ] 7.2 If extern funcs present, emit `#include "cactus_runtime.hpp"` after the fixed system includes
