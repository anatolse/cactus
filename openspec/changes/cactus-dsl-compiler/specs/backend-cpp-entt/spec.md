...
- **WHEN** a system has `filter: [Position, Velocity]` and an `on tick(dt: float):` handler
...
- **WHEN** the decorated AST contains `unit Cactus:` with `apply: [Position, Renderable]` and `config:` setting position values
