...
public:
    ...
    DataFileWriter(const ProgramNode& ast, const DecoratedProgram& decorated, ErrorReporter& errors);

    /// Write the data file. Returns the list of entity instances for testing.
    std::vector<EntityInstanceData> write(const std::filesystem::path& build_dir, const std::string& module_name);
