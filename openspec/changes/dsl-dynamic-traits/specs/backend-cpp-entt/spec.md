...
- A bare marker `add TraitName` on self compiles to `registry.emplace_or_replace<TraitName>(entity)`
- A field-bearing `add TraitName:` statement compiles to `registry.emplace_or_replace<TraitName>(entity, TraitName{.field = val, ...})`
- An `add TraitName to target_expr` statement compiles to `registry.emplace_or_replace<TraitName>(target_expr, ...)`
- Trait fields not supplied in an `add` statement that have defaults SHALL use the default value in the generated constructor
- Trait fields not supplied in an `add` statement that are already present on the component SHALL be patched only for the supplied fields; remaining fields retain their current values via get-then-replace pattern
...
- **WHEN** `add Stunned:` with `duration = 2.0` is compiled and `Stunned` has `var duration: float = 0.0`
...
With `ApplyEntry.initially_active` removed, all `apply:` entries SHALL compile to direct component initialization at entity spawn time. No conditional masks or activation flags are generated.
