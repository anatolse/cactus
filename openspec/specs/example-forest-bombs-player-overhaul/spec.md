...
- **WHEN** `forest_bombs.cactus` is read
- **THEN** a `SphereMesh` asset is declared with path `"models/sphere_lowpoly.mesh"` and the `BombTemplate` and `ThrowSystem` spawn reference it
...
- **THEN** the `PlayerMoveSystem` applies a negative yaw delta to `Player1`'s and `Player1Camera`'s `WorldTransform.rotation` each tick at a rate governed by `TURN_SPEED`

#### Scenario: Arrow left/right yaws Player 2
- **WHEN** Player 2's Right arrow key is held
- **THEN** the `PlayerMoveSystem` applies a positive yaw delta to `Player2`'s and `Player2Camera`'s `WorldTransform.rotation` each tick
...
- **THEN** the `PlayerMoveSystem` moves the player along their facing direction (world +X), not world -Z

#### Scenario: Up/Down arrows move Player 2 in their facing direction
- **WHEN** Player 2's Up arrow key is held
- **THEN** the `PlayerMoveSystem` advances Player 2's position in the XZ projection of their current facing direction
...
- **THEN** the `PlayerMoveSystem` adds `fly * FLY_SPEED * tick.dt` to `position.y` for both the player body and camera entity
...
The split-screen forest bombs example SHALL place each player's camera entity directly above the player body at `EYE_HEIGHT` with no back-offset (`CAMERA_BACK` constant removed), and the camera's initial rotation SHALL be `quat.identity()` so yaw deltas from the shared `PlayerMoveSystem` accumulate correctly.
