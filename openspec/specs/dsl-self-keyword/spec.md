...
The language SHALL provide `self` as a reserved keyword expression. Inside a unary entity-selected rule event handler, `self` SHALL evaluate to the `entity_id` currently being processed. `self` SHALL be unavailable in selectionless and pair handlers because neither domain has one unique current entity.

#### Scenario: `self` used as destroy target
- **WHEN** a unary entity rule handler contains `destroy self`
...
- **WHEN** a unary entity rule handler assigns `Parent.parent = self`
...
The language SHALL reject `self` outside rule event handlers. `self` is not available in top-level declarations, unit/template initialization, or pure `func` bodies.

#### Scenario: `self` in pure function rejected
- **WHEN** a `func` body returns `self`
- **THEN** the compiler reports an error indicating that `self` is only allowed inside rule event handlers
