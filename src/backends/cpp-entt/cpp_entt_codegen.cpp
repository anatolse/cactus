...
std::string CppEnttCodegen::generate(const DecoratedProgram& program) { // NOLINT(readability-function-cognitive-complexity)
    ...
    if (program.ast != nullptr) {
        ...
        out << "void cactus::runtime::entt_backend::generated_setup_dispatcher(entt::dispatcher& dispatcher) {\n";
        out << "    (void)dispatcher;\n";
        for (auto& decl : program.ast->declarations) {
            if (auto* event = std::get_if<EventNode>(&decl)) {
    ...
    out << "// ── Runtime Glue ────────────────────────────────────────────────────\n\n";
    out << "cactus::runtime::entt_backend::ProjectConfig cactus::runtime::entt_backend::generated_project_config() noexcept {\n";
    out << "    return {" << win_width << ", " << win_height << ", " << win_title << ", " << win_fps << "};\n";
    out << "}\n\n";

    out << "void cactus::runtime::entt_backend::generated_init_project(entt::registry& registry) {\n";
    if (program.ast != nullptr) {
        for (auto& decl : program.ast->declarations) {
            if (auto* unit = std::get_if<UnitNode>(&decl)) {
    ...
    out << "void cactus::runtime::entt_backend::generated_update_project(entt::registry& registry, entt::dispatcher& dispatcher, float dt) {\n";
    ...
    out << "void cactus::runtime::entt_backend::generated_render_project(entt::registry& registry, entt::dispatcher& dispatcher) {\n";
    out << "    (void)dispatcher;\n";
    if (program.ast != nullptr) {
        for (auto& decl : program.ast->declarations) {
            if (auto* sys = std::get_if<ExternSystemNode>(&decl)) {
